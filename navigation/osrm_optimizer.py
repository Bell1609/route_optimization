...
def trip_optimizer():
    ...
    # Function to create the trip map
    ...
    # Function to display trip statistics
    ...
    st.title('OSRM Optimizer')
    st.info("This app provides an optimal-like trip to visit all points at least once.")
    ...
    with col2:
        ...
        optimize_points = st.toggle("Optimize points", value=False)
        ...
        if verify_coverage:
            max_distance = st.slider("Maximum distance for point coverage (meters)", min_value=1, max_value=100, value=st.session_state.max_distance, step=1, key='max_distance', on_change=update_uncovered_points)
        else:
            max_distance = None
    ...
    if points_file is not None:
        ...
        if not all(points.geometry.type == 'Point'):
            ...
            num_points = st.slider("Number of points to use", min_value=1, max_value=len(points), value=min(100, len(points)))
            filtered_points = points.head(num_points)

            with st.expander("View Uploaded Points"):
                m = folium.Map(location=[filtered_points.geometry.y.mean(), filtered_points.geometry.x.mean()], zoom_start=10)
                for idx, row in filtered_points.iterrows():
            ...
            if select_specific_points:
                ...
                start_index = st.selectbox("Select start point:", range(len(filtered_points)), format_func=lambda x: f"Point {x}: ({filtered_points.iloc[x].geometry.y:.6f}, {filtered_points.iloc[x].geometry.x:.6f})")
                start_point = filtered_points.iloc[start_index].geometry

                end_index = st.selectbox("Select end point:", range(len(filtered_points)), format_func=lambda x: f"Point {x}: ({filtered_points.iloc[x].geometry.y:.6f}, {filtered_points.iloc[x].geometry.x:.6f})")
            ...
            if st.button("Start Trip Calculation") or st.session_state.trip_calculated:
                ...
                if st.session_state.trip_gdf is not None and not st.session_state.trip_gdf.empty:
                    with st.expander("View Calculated Trip", expanded=True):
                        ...
                        m = create_trip_map(st.session_state.trip_gdf, filtered_points, st.session_state.uncovered_points, verify_coverage)
                        folium_static(m)
                        display_trip_statistics(st.session_state.trip_gdf, profile, verify_coverage, st.session_state.uncovered_points, max_distance)
