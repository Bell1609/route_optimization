...
def main():
    ...
    default_locations = [['Point 1'],['Point 2'],['Point 3'],['...']]
    existing_locations = '\n'.join([x[0] for x in default_locations])
    selected_value = st.text_area("Enter Locations or Coordinates:", value=existing_locations)
    ...
    if st.button("Calculate Optimal Route"):
        ...
        if locations:
                ...
                if solution:
                    ...
                    st.write('\n')
                    st.write('[Google Maps Link Here]({})'.format(gmap_places))
                else:
